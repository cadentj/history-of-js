// Toy Browserify-shaped bundler: walk require() graph from entry, emit one file.
// Usage: node bundler.js src/main.js > bundle.js
var fs = require('fs');
var path = require('path');

function findRequireSpecs(source) {
  var specs = [];
  var re = /require\(\s*['"]([^'"]+)['"]\s*\)/g;
  var m;
  while ((m = re.exec(source))) specs.push(m[1]);
  return specs;
}

function resolveRelative(spec, callerDir) {
  var base = path.resolve(callerDir, spec);
  var tries = [base, base + '.js', path.join(base, 'index.js')];
  for (var i = 0; i < tries.length; i++) {
    try { if (fs.statSync(tries[i]).isFile()) return tries[i]; } catch (e) {}
  }
  throw new Error('Cannot resolve ' + spec + ' from ' + callerDir);
}

/*TODO:
  Walk the require() graph starting at entryPath:
    - Assign each unique absolute path an integer id (start at 0).
    - For each module: record its source, and a { spec: id } map for its require calls.
    - Queue newly discovered deps and process them.
  Return { modules: { id: { code, deps } }, entryId }.
*/
// BEGIN:SOLUTION
function buildGraph(entryPath) {
  var modules = {};
  var byPath = {};
  var queue = [path.resolve(entryPath)];
  var nextId = 0;

  while (queue.length) {
    var abs = queue.shift();
    if (abs in byPath) continue;
    var id = nextId++;
    byPath[abs] = id;
    var code = fs.readFileSync(abs, 'utf8');
    var deps = {};
    var callerDir = path.dirname(abs);
    findRequireSpecs(code).forEach(function (spec) {
      var depAbs = resolveRelative(spec, callerDir);
      deps[spec] = depAbs;
      if (!(depAbs in byPath)) queue.push(depAbs);
    });
    modules[id] = { code: code, deps: deps };
  }

  for (var id in modules) {
    var mapped = {};
    for (var spec in modules[id].deps) mapped[spec] = byPath[modules[id].deps[spec]];
    modules[id].deps = mapped;
  }
  return { modules: modules, entryId: byPath[path.resolve(entryPath)] };
}
// END:SOLUTION

/*TODO:
  Emit the bundle:
    - For each module, wrap its source in:
        function (module, exports, require) { <source> }
    - Rewrite each require("./foo") → require(<id>) using the deps map.
    - Ship a tiny runtime that calls require(entryId).
*/
// BEGIN:SOLUTION
function rewriteRequires(code, deps) {
  return code.replace(/require\(\s*['"]([^'"]+)['"]\s*\)/g, function (_, spec) {
    return 'require(' + deps[spec] + ')';
  });
}

function emit(graph) {
  var entries = Object.keys(graph.modules).map(function (id) {
    var m = graph.modules[id];
    return id + ': function (module, exports, require) {\n' +
      rewriteRequires(m.code, m.deps) +
      '\n}';
  });
  var RUNTIME =
    '(function () {\n' +
    '  var modules = { ' + entries.join(', ') + ' };\n' +
    '  var cache = {};\n' +
    '  function require(id) {\n' +
    '    if (cache[id]) return cache[id].exports;\n' +
    '    var module = { exports: {} };\n' +
    '    cache[id] = module;\n' +
    '    modules[id](module, module.exports, require);\n' +
    '    return module.exports;\n' +
    '  }\n' +
    '  require(' + graph.entryId + ');\n' +
    '})();\n';
  return RUNTIME;
}
// END:SOLUTION

var entry = process.argv[2];
if (!entry) {
  console.error('usage: node bundler.js <entry>');
  process.exit(1);
}
process.stdout.write(emit(buildGraph(entry)));
